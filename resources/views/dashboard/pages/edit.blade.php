...
<body class="vertical-layout vertical-menu-modern 2-columns   menu-expanded fixed-navbar" data-open="click"
    ...
    <div class="app-content content mt-5">
        ...
        <form class="my-5 w-75 container" id="coupon_form" method="POST" action="{{route('pages.update',$page)}}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-body">
                <div class="form-group">
                    <label for="title_ar">{{ __('sliders.titel_ar') }}</label>
                    <input type="text" id="title_ar" class="form-control"name="title[ar]" value="{{$page->getTranslation('title','ar')}}">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="title_en" class="form-control" name="title[en]" value="{{$page->getTranslation('title','en')}}">
               ...
               @enderror
                <div class="form-group">
                    ...
                    <textarea id="summernote_ar" class="container"name="content[ar]">{!! $page->getTranslation('content', 'ar') !!}</textarea>
                </div>
                @error('content.ar')
                <span class="text-danger">{{$message}}</span>
               @enderror
                <div class="form-group">
                    ...
                    <textarea id="summernote_en" class="container" style="direction: ltr;"name="content[en]">{!! $page->getTranslation('content', 'en') !!}</textarea>
                </div>
                @error('content.en')
                <span class="text-danger">{{$message}}</span>
               @enderror
