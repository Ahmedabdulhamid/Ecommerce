                            ...
                            <div class="product-info-img">
                                ...
                                <div class="swiper product-top">
                                    <div class="swiper-wrapper">
                                        @foreach ($product->images as $image)
                                            <div class="swiper-slide slider-top-img">
                                                <img src="{{ asset('storage/products/' . $image->file_name) }}"
                                    ...
                                    @if ($product->has_discount)
                                        <div class="product-discount-content" style="z-index: 100;">
                                            <h4>
                                ...
                                <div class="swiper product-bottom mt-3">
                                    <div class="swiper-wrapper">
                                        @foreach ($product->images as $image)
                                            <div class="swiper-slide slider-bottom-img">
                                                <img src="{{ asset('storage/products/' . $image->file_name) }}"
                                                    alt="{{ $product->getTranslation('name', 'en') }}" width="80px">
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
...
<p class="content-paragraph">
    {{ $product->getTranslation('small_desc', app()->getLocale()) }}
...
@if ($variantArray)
    @foreach ($variantArray as $name => $value)
        <p>{{ $name }} : {{ $value }}</p>
    @endforeach
@endif
                    ...
                    <div class="size-info ">
                        <div class="size-info d-flex justify-content-between">
                            @foreach ($variant->product_attributes as $attr)
                                @if ($attr->attr_value->attribute->getTranslation('name', 'en') == 'size')
                                    <div>
                                        <span>{{ $attr->attr_value->attribute->getTranslation('name', app()->getLocale()) }}:</span>
                                        <span>{{ $attr->attr_value->value }}</span>

                                    </div>
                                @endif
                                @if ($attr->attr_value->attribute->getTranslation('name', 'en') == 'color')
                                    <div>
                                        <span>{{ $attr->attr_value->attribute->getTranslation('name', app()->getLocale()) }}:</span>
                                        <span>{{ $attr->attr_value->value }}</span>
...
<div class="product-quantity">
    <div class="quantity-wrapper">
        <div class="quantity">
            <a href="#"wire:click.prevent='decrementQuantity' class="minus">
                -
            </a>
            <span class="number">{{ $quantity }}</span>
            <a href="#"wire:click.prevent='incrementQuantity' class="plus">
                +
            </a>
        </div>
        <div class="wishlist">
            <a href="#" wire:click.prevent='AddToWatchList'>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"class="text-danger"
                    xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M17 1C14.9 1 13.1 2.1 12 3.7C10.9 2.1 9.1 1 7 1C3.7 1 1 3.7 1 7C1 13 12 22 12 22C12 22 23 13 23 7C23 3.7 20.3 1 17 1Z"
                        stroke="@if ($isInWatchlist == true) currentColor
                         @else
                         #797979 @endif
                        "
...
<div class="product-share">
    ...
    <div class="social-icons">
        ...
        <a href="https://twitter.com/intent/tweet?url={{ $url }}"target="_blank" rel="noopener">
            <span class="twitter">
...
@include('front.products.tes')
</div>
<script>
    window.addEventListener('CheckUserLogin', function() {
    ...
    window.addEventListener('success', function(msg) {
        console.log(msg.detail[0]);

        toastr.success(msg.detail[0])

    })
    window.addEventListener('updated', function(msg) {
        console.log(msg.detail[0]);

        toastr.info(msg.detail[0])

    })
    window.addEventListener('sendSuccessMsg', function() {
...
<script>
    function initSwipers() {
        ...
        if (window.mySwiperTop) window.mySwiperTop.destroy(true, true);
        if (window.mySwiperBottom) window.mySwiperBottom.destroy(true, true);
        console.log('hello');

        // إنشاء Swiper جديد للصور الكبيرة
        window.mySwiperTop = new Swiper('.product-top', {
            spaceBetween: 10,
            loop: true,
            slidesPerView: 1,
        });

        // إنشاء Swiper جديد للصور الصغيرة
        window.mySwiperBottom = new Swiper('.product-bottom', {
            spaceBetween: 10,
            slidesPerView: 4,
            slideToClickedSlide: true, // هذا السطر مهم لتمكين التفاعل بين الصور الصغيرة والكبيرة
            breakpoints: {
                // يمكن أن تضيف إعدادات مختلفة للأجهزة المختلفة هنا
                768: {
                    slidesPerView: 5, // لزيادة عدد الصور الصغيرة الظاهرة في الشاشة الكبيرة
                },
            },
        });

        // ربط الـ swiper الكبير بالـ swiper الصغير
        window.mySwiperTop.controller.control = window.mySwiperBottom;
        window.mySwiperBottom.controller.control = window.mySwiperTop;
    }
    window.addEventListener('swiper-update', () => {
        initSwipers(); // دي الدالة اللي فيها إعدادات Swiper
    });
    window.addEventListener('added_successfully', function() {
        toastr.success('You Added This Product To Watchlist Successfully!')
    })
