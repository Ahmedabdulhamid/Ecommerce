          ...
          <div class="sidebar" >
              <div class="sidebar-section">
                  ...
                  <div class="sidebar-wrapper">
                      <h5 class="wrapper-heading{">{{__('front.product_categories')}}</h5>
                              ...
                              @foreach ($categories as $cat)
                                  <li>
                                      ...
                                      <div wire:key="cat-{{ $cat->id }}">
                                          <input type="checkbox" wire:model.live="categoryIds"
                                              value="{{ $cat->id }}">

                                          <label>{{ $cat->getTranslation('name', app()->getLocale()) }}</label>
                                      </div>
                                  </li>
                              @endforeach
                  ...
                  <div class="sidebar-wrapper sidebar-range">
                      <h5 class="wrapper-heading">{{__('front.price_range')}}</h5>

                      <div class="price-slide range-slider">
                          <div class="mb-2">
                              <span>EGP <span id="min-val">{{ number_format($minPrice, 2) }}</span></span>
                              <span>-</span>
                              <span>EGP <span id="max-val">{{ number_format($maxPrice, 2) }}</span></span>
                          </div>

                          <input type="range" min="{{ $minPrice }}" max="{{ $maxPrice }}" step="1"
                              id="min-range" wire:model.live='minPrice' value="{{$minPrice}}">

                          <input type="range" min="{{ $minPrice }}" max="{{ $maxPrice }}" step="1"
                              id="max-range" wire:model.live='maxPrice' value="{{$maxPrice}}">
                      </div>
                  </div>
                  ...
                  <hr>
                  <div class="sidebar-wrapper">
                      <h5 class="wrapper-heading">{{__('admin.brand')}}</h5>
                      <div class="sidebar-item">
                          <ul class="sidebar-list">
                              @foreach ($brands as $brand)
                                  <li>
                                      <div wire:key="brand-{{ $brand->id }}">
                                          <input type="checkbox" wire:model.live="brandsIds"
                                              value="{{ $brand->id }}">
                                          <label>{{ $brand->getTranslation('name', app()->getLocale()) }}</label>
                                      </div>
                                  </li>
                              @endforeach
              ...
              <div class="row g-5">
                  ...
                  <div class="col-lg-12">
                      <div class="product-deal-section"
                          style="background-image:url({{ asset('front-assets/images/homepage-one/product-deal-.webp') }})">
                          <h5 class="wrapper-heading">{{__('front.sentence_shop')}}</h5>
                          <a href="{{url()->current()}}" class="shop-btn">{{__('front.shop_now')}}</a>
                      </div>
                  </div>
                  <div wire:loading.delay class="position-fixed top-0 start-0 w-100 h-100 bg-white opacity-75 z-50">
  ...
  <script>
      document.addEventListener('DOMContentLoaded', function() {
          ...
          function updatePrices() {
              ...
              // Update text
              minVal.innerText = min.toFixed(2);
              maxVal.innerText = max.toFixed(2);
